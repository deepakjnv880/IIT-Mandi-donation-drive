...
def signupforstudent():
  ...
  #  epass1 = generate_password_hash(password1)
    password2=request.form['password2']
    if (password2==password1):
        epass = generate_password_hash(password1)

    #epass2 = generate_password_hash(password2)
    print(name,email,type,password1,password2,epass)
...
def signinstudent():
   ...
   # print(query_string," +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ",password)
    # con.commit()
    #result = check_password_hash(query_string, password)
        
    #if(str(result) == true):
    #    return redirect(url_for('index'))

    if (check_password_hash(query_string[0][0], password)== 'False' ):
        return redirect(url_for('index'))
    else:
        return redirect(url_for('account',username=x,type="student",email=email))
